...
def parse_args():
    ...
    parser.add_argument('--source', type=str, required=True, help='source image file path')
...
def yolov10_autolabel(args):
    ...
    if img_file.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp', '.tiff')):
        ...
        if bbox_list:  # Only save if there are valid bounding boxes
            labelfilename = os.path.join(args.label, os.path.splitext(os.path.basename(img_file))[0] + '.txt')
            ...
            labeled_image_path = os.path.join(args.label, os.path.basename(img_file))
